...
func TestTagNames(t *testing.T) {
	...
	tagNames, err := cli.TagNames("library/ubuntu")

	assert.NotZero(len(tagNames))
	assert.Nil(err)
}

func TestTagNamesForInvalidRepo(t *testing.T) {
	assert := assert.New(t)

	cli, _ := New(registry, Config{})

	tagNames, err := cli.TagNames("idonotexist/omg")
...
func TestTag(t *testing.T) {
	...
	tg, err := cli.Tag("library/ubuntu", "latest")

	assert.Equal(40, len(tg.GetShortDigest()))
	assert.Equal(12, len(tg.GetImageID()))
	assert.True(tg.GetCreated() > 1532643000)
	assert.Nil(err)
}
