...
type RegistryClient struct {
	...
	RepoTokens map[string]token.Token
}
...
func New(registry string, config Config) (*RegistryClient, error) {
	...
	return &RegistryClient{
		registry:   registry,
		Config:     config,
		RepoTokens: make(map[string]token.Token),
	}, nil
}
...
func (cli *RegistryClient) Login(username, password string) error {
	tk, err := token.New(cli.URL(), username, password, "registry:catalog:*")
	if err != nil {
...
func (cli *RegistryClient) repoToken(repoPath string) (token.Token, error) {
